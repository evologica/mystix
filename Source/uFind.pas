...
begin
    ...
    begin
    	if DocumentFactory.IsSearchedForTheFirstTime( PChar(frFindText) ) then
        	...
        	frMatchCase, frRegExp, chkSelOnly.Checked, frFromCursor, frDirUp)
      else
  ...
  begin
    ...
    begin
			if not Document.FindText(frFindText, frWholeWords,
				frMatchCase, frRegExp, chkSelOnly.Checked, frFromCursor, frDirUp) then
        	NotFound;
    end
    else
    begin
...
procedure TFindDialog.ReadLanguageData;
var
	i: Integer;
begin
	with TMyIniFile.Create(Languages.Values[ActiveLanguage]) do
  try
  	Caption := ReadString('FindDialog', 'Caption', '');

  	for i := 0 to ComponentCount - 1 do
    	if (Components[i] is TControl) then
				(Components[i] as TControl).SetTextBuf(PChar(ReadString('FindDialog',
