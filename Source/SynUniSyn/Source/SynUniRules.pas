...
begin
  ...
  while (xml.CurPartType <> ptStartTag) or not SameText(xml.CurName, 'Keywords') do xml.Scan;
...
//    Attribs := fStyles.GetStyleDef(getAttrValue('style', xml), defaultattr);
  ...
  else raise Exception.Create(ClassName + '.LoadFromXml - no keywords to load!');

  KeyList.BeginUpdate;
  KeyList.Clear;
  try
    while xml.Scan do
      if xml.CurPartType = ptEmptyTag then begin
      ...
      if xml.CurPartType = ptEndTag then if SameText(xml.CurName, 'Keywords') then
        Break;
  finally
    KeyList.EndUpdate;
  end;
...
begin
  if xml = nil then Exit;
  while (xml.CurPartType <> ptEmptyTag) or not SameText(xml.CurName, 'Set') do xml.Scan;
...
//    Attribs := fStyles.GetStyleDef(getAttrValue('style', xml), defaultattr);
  ...
  else raise Exception.Create(ClassName + '.LoadFromXml - no set to load!');
end;
...
begin
  if xml = nil then Exit;
  while (xml.CurPartType <> ptStartTag) or not SameText(xml.CurName, 'Range') do xml.Scan;
...
//        TermSymbols := String2Set(xml.CurContent) + [#32, #9, #13, #10] else
  ...
  else raise Exception.Create(ClassName + '.LoadFromXml - no range to load!');
